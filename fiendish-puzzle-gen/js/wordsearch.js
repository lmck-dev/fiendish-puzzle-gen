/* =============================================
   Fiendish Puzzle Gen — Word Search Logic
   ============================================= */
...
(function () {
    ...
    // ---- State ----
    let words = [];
    let lastPuzzleData = null; // stores { grid, placements, size } for answer key
    ...
    const showAnswerKeyToggle = document.getElementById("show-answer-key");
    ...
    function renderPuzzle(grid, targetTable) {
    ...
    // ---- UI: Render answer key grid ----
    function renderAnswerKey(grid, placements, size, targetTable) {
        targetTable.innerHTML = "";

        // Build a set of cells that are part of placed words
        ...
        for (let r = 0; r < size; r++) {
            ...
            for (let c = 0; c < size; c++) {
                ...
                if (highlightCells.has(key)) {
                    td.classList.add("answer-highlight");
                } else {
                    td.classList.add("answer-filler");
                }
                tr.appendChild(td);
            }
            targetTable.appendChild(tr);
    ...
    function generate() {
        ...
        const { grid, placements } = createWordSearch([...words], size, mode);

        // Store for answer key re-renders
        lastPuzzleData = { grid, placements, size };

        // Render puzzle grid
        renderPuzzle(grid, puzzleGrid);
        renderWordBank(words);
        printTitle.textContent = title;

        // Render answer key if enabled
    ...
    function updateAnswerKeyVisibility() {
        if (!lastPuzzleData) return;

        const showKey = showAnswerKeyToggle.checked;
        if (showKey) {
            renderAnswerKey(
                lastPuzzleData.grid,
                lastPuzzleData.placements,
                lastPuzzleData.size,
                answerGrid
            );
    ...
    // Answer key toggle — live update when toggled after generation
    showAnswerKeyToggle.addEventListener("change", updateAnswerKeyVisibility);
